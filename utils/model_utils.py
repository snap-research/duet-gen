...
from utils.transformations import aa2d6, d62aa, d62rotmat
...
@torch.no_grad()
def fullpose2smplx_params_aa(pose,trans, to_torch=False):
    if to_torch:
        global_orient = torch.from_numpy(pose[..., :3]).type(torch.float32).to(device)
        body_pose = torch.from_numpy(pose[..., 3:66]).type(torch.float32).to(device)
        jaw_pose  = torch.from_numpy(pose[..., 66:69]).type(torch.float32).to(device)
        leye_pose = torch.from_numpy(pose[..., 69:72]).type(torch.float32).to(device)
        reye_pose = torch.from_numpy(pose[..., 72:75]).type(torch.float32).to(device)
        left_hand_pose = torch.from_numpy(pose[..., 75:120]).type(torch.float32).to(device)
        right_hand_pose = torch.from_numpy(pose[..., 120:]).type(torch.float32).to(device)
        trans = torch.from_numpy(trans).type(torch.float32).to(device)
    else:
