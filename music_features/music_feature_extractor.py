...
class FeatureExtractor:
    @staticmethod
    def get_melspectrogram(audio, sample_rate, n_mels=128, fmax=8000):
        melspe = librosa.feature.melspectrogram(y=audio, sr=sample_rate, n_mels=n_mels, fmax=fmax)
...
def signal_to_feature(audio, sr):
    ...
    onset_strength = extractor.get_onset_strength(audio_percussive, sr)
    tempogram = extractor.get_tempogram(onset_strength, sr)
    onset_beat = extractor.get_onset_beat(onset_strength, sr)[0]

    onset_strength = onset_strength.reshape(1, -1)
