...
from train.args_masked_transformer_top import arg_parse
...
class MaskedTransformerTrainer:
    def __init__(self, args):
        ...
        self.vq_model, self.vq_opt = self.load_vq_model(self.opt.vq_pretrained_weight_path)
        ...
        if self.opt.is_train:
            ...
            # self.z_normalization = self.train_loader.dataset.z_normalization
            ...
            self.data_loader = DataLoader(self.load_dataset, batch_size=1, pin_memory=False,
                                        ...
                                        shuffle=False)
            # self.z_normalization = self.data_loader.dataset.z_normalization
    ...
    def forward(self, batch_data, iterations, teacher_forcing):
        ...
        if self.opt.predict_velocity:
            ...
            # for training hierarchical top codes 
            pred_code, ce_loss, acc  = self.transformer_model(code_idx[1],
    ...
    def test(self): 
        ...
        for running_iter, batch_data in enumerate(self.data_loader):
            ...
            pred_topcode_idx  = self.transformer_model.generate(batch_data['music_feats_mfcc'],
                                                                    batch_data['music_feats_chroma'],
                                                                    token_len=50,
                                                                    timesteps=20
                                                                    )
            
            pred_code_idx = [code_idx[0], pred_topcode_idx]
            pred_motion_delta = self.vq_model.decode(pred_code_idx, batch_data['genre_class'], # we use zeros for genre in the decode function
                                                     batch_data['music_feats_mfcc'], batch_data['music_feats_chroma'])
